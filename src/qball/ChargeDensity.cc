...
void ChargeDensity::plot_rhoint(const Sample * s, const std::string & filename) const {
    using namespace std;

    std::vector<double> rho;
    vft()->gather(*s->wf.spincontext(0), rhor[0], rho);
    ...
    if ( s->ctxt_.onpe0() ) {
        ...
        os.open((filename + ".rhoz").c_str(), ofstream::out);
        ...
        for ( int k = 0; k < np2; k++ ) {
            ...
            for ( int j = 0; j < np1; j++ ) {
                ...
                for ( int i = 0; i < np0; i++ ) {
                    ...
                    rhoz += rho[ip + np0*(jp + np1*kp)]*surface_element;
                }
            }
            os<<rhoz<<endl;
        }
